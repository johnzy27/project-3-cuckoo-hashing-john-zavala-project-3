...
int main() {
  ...
  while ( getline(infile,s) ) {
    ...
    //BEFORE THIS WAS S[len-1] I changed to s[len]
...
size_t f(string s, size_t index) {
  ...
  else 
  {
    ...
    // HASH FUNCTION in <val>
      ...
      //cout << "VAL OUTPUT: " << val << endl << endl;
      
      //setting val = val mod 17
      val = val % tablesize;

      if (val < 0) 
      {
         // cout << "Inside first val if statment\n";
          val = val + tablesize;
      }
      
      //returning val if len equals 1
      if (len == 1)  return val;
   
      
      //looping until len i <= len
      for (i = 2; i <= len; i++)
      {
          temp = s[len - i];

          //calculating and setting it = po
          po = po * prime;

          //setting po equal to po mod 17
          po = po % tablesize;

          //if statment checking if po is less than 0 then computing
          if (po < 0) po += tablesize;
          ...
          //if statment checking if val is less than 0 then computing
          if (val < 0) val += tablesize;
      }
